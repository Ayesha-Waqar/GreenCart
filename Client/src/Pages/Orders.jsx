  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8">
      ...
      <div className="space-y-6">
        {myOrders.map((order) => (
          <div key={order._id} className="border border-gray-200 rounded-lg overflow-hidden shadow-sm bg-white">
            {/* Order Header */}
            ...
            <div className="divide-y divide-gray-100">
              {order.items && order.items.map((item, index) => (
                <div key={index} className="p-4 flex flex-col md:flex-row md:items-center justify-between gap-6">
                  ...
                  <div className="flex flex-col md:items-end justify-center">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="text-sm font-medium text-gray-700">{item.status || "Processing"}</p>
                    </div>
                    <p className="text-xs text-gray-400">
                      Ordered on: {new Date(order.createdAt).toLocaleDateString()}
                    </p>
                    <p className="mt-2 font-semibold text-gray-800">
                      Subtotal: {currency}{item.product?.offerPrice * (item.quantity || 1)}
                    </p>
                  </div>
                </div>
              ))}
